...
func (c *ovnDBClient) validate() error {
	var errs []error
	if c.listenAddr == "" {
...
func (c *ovnDBClient) run(ctx context.Context) error {
	/* Listen TLS
...
func (c *ovnDBClient) copyEgress(closer chan struct{}, dst io.Writer, src net.Conn) {
	/* TLS conn
	...
	*/
	...
	// TCP conn
	req, err := http.NewRequest("GET", c.ovnDBAddr, src)
	if err != nil {
		fmt.Errorf("failed to read request from local conn: %s", err)
	}
	fmt.Println("message from local conn: %v", req)
	req.Host = c.ovnDBAddr
	req.TLS = &tls.ConnectionState{
		ServerName: c.ovnDBAddr,
	}
	fmt.Println("message after modification: %v", req)
...
func (c *ovnDBClient) copyIngress(closer chan struct{}, dst io.Writer, src io.Reader) {
