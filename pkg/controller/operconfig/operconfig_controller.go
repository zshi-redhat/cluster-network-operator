...
func (r *ReconcileOperConfig) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	ctx = context.WithValue(ctx, names.KEY_CONTROLLER_NAME, "operconfig")
	...
	for i, obj := range objs {
		...
		if err := apply.ApplyObject(ctx, r.client.ClientFor(obj.GetClusterName()), obj, "operconfig"); err != nil {
	...
	if status != nil {
		...
		if err := apply.ApplyObject(ctx, r.client.ClientFor(status.GetClusterName()), status, "operconfig"); err != nil {
