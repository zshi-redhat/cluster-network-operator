...
func (status *StatusManager) SetFromPods() {
	...
	for _, dsName := range status.daemonSets {
		ds := &appsv1.DaemonSet{}
		if err := status.client.Default().CRClient().Get(context.TODO(), dsName, ds); err != nil {
		...
		if isNonCritical(ds) && ds.Status.NumberReady == 0 && !status.installComplete {
			...
			if !isNonCritical(ds) {
				hung = append(hung, status.CheckCrashLoopBackOffPods(dsName, ds.Spec.Selector.MatchLabels, "DaemonSet", cnoclient.DefaultClusterName)...)
			}
	...
	for _, ssName := range status.statefulSets {
		...
		if err := status.client.ClientFor(cnoclient.ManagementClusterName).CRClient().Get(context.TODO(), ssName, ss); err != nil {
		...
		if isNonCritical(ss) && ss.Status.ReadyReplicas == 0 && !status.installComplete {
			...
			// Check for any pods in CrashLoopBackOff state and mark the operator as degraded if so.
			if !isNonCritical(ss) {
				hung = append(hung, status.CheckCrashLoopBackOffPods(ssName, ss.Spec.Selector.MatchLabels, "StatefulSet", cnoclient.ManagementClusterName)...)
			}
	...
	for _, depName := range status.deployments {
		...
		if err := status.client.Default().CRClient().Get(context.TODO(), depName, dep); err != nil {
		...
		if isNonCritical(dep) && dep.Status.UnavailableReplicas > 0 && !status.installComplete {
			...
			if !isNonCritical(dep) {
				hung = append(hung, status.CheckCrashLoopBackOffPods(depName, dep.Spec.Selector.MatchLabels, "Deployment", cnoclient.DefaultClusterName)...)
			}
...
func (status *StatusManager) getLastPodState() (map[types.NamespacedName]daemonsetState, map[types.NamespacedName]deploymentState, map[types.NamespacedName]statefulsetState) {
	// with maps allocated
	daemonsetStates := map[types.NamespacedName]daemonsetState{}
	deploymentStates := map[types.NamespacedName]deploymentState{}
	statefulsetStates := map[types.NamespacedName]statefulsetState{}
	...
	for _, ss := range out.StatefulsetStates {
		statefulsetStates[ss.NamespacedName] = ss
	}
...
func (status *StatusManager) setLastPodState(
	...
	sss map[types.NamespacedName]statefulsetState) error {
	...
	return retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		err := status.client.Default().CRClient().Get(context.TODO(), types.NamespacedName{Name: status.name}, oldStatus)
		...
		return status.client.Default().CRClient().Patch(context.TODO(), newStatus, crclient.MergeFrom(oldStatus))
	})
}
...
func (status *StatusManager) CheckCrashLoopBackOffPods(name types.NamespacedName, selector map[string]string, kind string, cluster string) []string {
	hung := []string{}
	pods := &v1.PodList{}
	err := status.client.ClientFor(cluster).CRClient().List(context.TODO(), pods, crclient.InNamespace(name.Namespace), crclient.MatchingLabels(selector))
	if err != nil {
...
func (status *StatusManager) setSSAnnotation(obj *appsv1.StatefulSet, key string, value *string) error {
	...
	new.SetAnnotations(anno)
	return status.client.ClientFor(cnoclient.ManagementClusterName).CRClient().Patch(context.TODO(), new, crclient.MergeFrom(obj))
}
